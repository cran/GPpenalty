...
# mle
mle_gp <- function(y, x, sep=TRUE, mu=FALSE, g=FALSE, fixed_g=NULL, profile=TRUE, initialvals=NULL, penalty=FALSE, scad=FALSE, lambda=0, ncores=1) {
  ...
  if(is.null(initialvals)) {
    ...
    if(profile & dim!=ncol(initialvals)) {
  ...
  if(penalty) {
    ...
    return(list(y=y, x=x, theta=estimates$theta, sigma2=estimates$s2, mu=estimates$mu, g=estimates$g, penalty=penalty, lambda=estimates$lambda))
  } else {
    estimates <- multi(object)
    return(list(y=y, x=x, theta=estimates$theta, sigma2=estimates$s2, mu=estimates$mu, g=estimates$g, penalty=penalty))
  }
